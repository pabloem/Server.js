...
function LinkedDataFragmentsServer(options) {
  ...
  // Attach filters
  if (options.filter)
    server._filter = _.assign(options.filter, { type: require('./filters/' + options.filter.type) });
...
LinkedDataFragmentsServer.prototype._createFragmentMetadata =
  ...
  // TEMP: temporary code for bloom filters
  var filterEnabled = (query.subject ? 1 : 0) + (query.predicate ? 1 : 0) + (query.object ? 1 : 0) === 2,
      self = this;
  // If only one variable present, create bloomfilter
  if (filterEnabled && this._filter) {
    var variable;
    ...
    queryResult.on('metadata', function (metadata) {
      ...
      self._filter.type(datasourceSettings.datasource, query, variable, totalCount, self._filter.probability, function (error, filterData) {
        if (error)
