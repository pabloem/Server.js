...
function LinkedDataFragmentsServer(options) {
  ...
  // Attach filters
  if (options.filter)
    server._filter = require('./filters/' + options.filter);
...
LinkedDataFragmentsServer.prototype._createFragmentMetadata =
  ...
  if (filterEnabled) {
    ...
    queryResult.on('metadata', function (metadata) {
      ...
      self._filter(datasourceSettings.datasource, query, variable, totalCount, error_p, function (error, filter) {
        if (!error)
          queryResult.emit('filter', filter);
      });
    });
  }
