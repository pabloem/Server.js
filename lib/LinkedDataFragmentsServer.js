...
LinkedDataFragmentsServer.prototype._createFragmentMetadata =
  ...
  if (filterEnabled) {
    ...
    queryResult.on('metadata', function (metadata) {
      ...
      var result = datasourceSettings.datasource.select(_.omit(query, ['limit', 'offset']), function (error) {
        console.log(error);
      });
      ...
      result.on('end', function () {
        queryResult.emit('filter', { variable: variable, filter: bloom.bitfield.buffer.toString('base64'), m: m, k: k });
      });
    });
  }
