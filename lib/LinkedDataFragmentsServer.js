...
var http = require('http'),
    ...
    base64ArrayBuffer = require('./util/base64ArrayBuffer');
  ...
  queryResult.on('metadata', function (metadata) {
    ...
    if (totalCount > 1 && totalCount < 10000) {
      ...
      result.on('data', function (triple) {
        var tripleUrl = url.format(_.defaults({ search: '', query: triple },
                                 requestUrl));
        bloom.add(tripleUrl);
      });
      result.on('end', function () {
          queryResult.emit('filter', base64ArrayBuffer(bloom.buckets));
      });
    } else
      queryResult.emit('filter');
  });
