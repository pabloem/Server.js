...
var dcTerms = 'http://purl.org/dc/terms/',
    ...
    amq = 'http://semweb.mmlab.be/ns/amq#';
...
TurtleWriter.prototype.writeFragment = function (destination, tripleStream, settings) {
  ...
  var writer = this._createWriter(destination, settings),
      metadataWritten = false, triplesWritten = false, filterWritten = false,
      datasource = settings.datasource, fragment = settings.fragment, query = settings.query, filterEnabled = settings.filterEnabled;
  ...
  tripleStream.on('metadata', function (metadata) {
    ...
    triplesWritten && (!filterEnabled || filterWritten) && writer.end();
  });

  tripleStream.on('filter', function (filter) {
    // Write AMQ filter
    if (filter) {
      writer.meta(fragment.url, amq + 'membershipFilter', '_:filter');

      writer.meta('_:filter', rdf + 'type', '"' + filter.type);
      writer.meta('_:filter', amq + 'filter', '"' + filter.filter + '"^^' + xsd + 'base64Binary');
      writer.meta('_:filter', amq + 'variable', '"' + filter.variable + '"');

      filter.p && writer.meta('_:filter', amq + 'probability', '"' + filter.p + '"^^' + xsd + 'float');
      filter.k && writer.meta('_:filter', amq + 'hashes', '"' + filter.k + '"^^' + xsd + 'integer');
      filter.m && writer.meta('_:filter', amq + 'bits', '"' + filter.m + '"^^' + xsd + 'integer');
    }

    // End the writer if all triples have been written
    filterWritten = true;
    triplesWritten && metadataWritten && writer.end();
  });

  // Add data triples
  tripleStream.on('data', function (t) { writer.data(t.subject, t.predicate, t.object); });
  tripleStream.on('end',  function ()  { triplesWritten = true; metadataWritten && (!filterEnabled || filterWritten) && writer.end(); });
};
