...
TurtleWriter.prototype.writeFragment = function (destination, tripleStream, settings) {
  ...
  tripleStream.on('filter', function (filter) {
    ...
    if (filter) {
      ...
      writer.meta(fragment.url, amq + 'filter', '"' + filter.filter + '"^^' + xsd + 'base64Binary');
      writer.meta(fragment.url, amq + 'variable', '"' + filter.variable + '"');
      writer.meta(fragment.url, amq + 'hashes', '"' + filter.k + '"^^' + xsd + 'integer');
      writer.meta(fragment.url, amq + 'bits', '"' + filter.m + '"^^' + xsd + 'integer');
    }
