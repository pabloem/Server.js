...
var dcTerms = 'http://purl.org/dc/terms/',
    ...
    awq = 'http://semweb.mmlab.be/ns/awq#';
...
TurtleWriter.prototype.writeFragment = function (destination, tripleStream, settings) {
  ...
  tripleStream.on('metadata', function (metadata) {
    ...
    triplesWritten && filterWritten && writer.end();
  });

  tripleStream.on('filter', function (filter) {
    // Write AMQ filter
    if (filter)
      writer.meta(fragment.url, awq + 'filter', '"' + filter + '"^^' + xsd + 'base64Binary');
  ...
  tripleStream.on('end',  function ()  { triplesWritten = true; metadataWritten && filterWritten && writer.end(); });
};
