...
function ChangesetManager(options) {
    ...
    this._CSET_THRESHOLD = options.cset_threshold || 0; // Maximum number of changesets to apply per cycle

    /* We keep these functions to allow unit testing of each one of the
...
ChangesetManager.prototype._getChangesetListAsync = function(url,callback) {
    ...
    request(url,function(error,response,body) {
        ...
        if(_this._received == _this._awaiting) {
            ...
            process.nextTick(function(){_this.afterListDownload();});
        }
    });
};
            ...
            cs.downloadAndParse(function(){
                ...
                if(_this._received == _this._awaiting) {
                    ...
                    if(_this._CSET_THRESHOLD && _this._CSET_THRESHOLD < _this._totalChangesets) {
                        process.nextTick(function(){_this.afterChangesetsDownload();});
                    }
                }
            });
        }
...
ChangesetManager.prototype._computeOperationList = function() {
    var cList = this._changesetLists,
        store = N3.Store(),
        ops = [],
        found = {'added': function(triple){}, 'removed': function(triple) { store.removeTriple(triple);}},
        not_found = {'added': function(triple){ store.addTriple(triple);},'removed': function(triple){}};
    for(var i = 0; i < cList.length; i++) {
        if(cList[i].csets === undefined) {
            ...
            // TODO - We might want to SLICE this._changesetList
            break;
        }
        var operation = cList[i].csets.getOperation(),
            triples = cList[i].csets.getTriples();
        for(var j = 0; j < triples.length; j++) {
            var tr = triples[j];
            if(store.find(tr.subject, tr.predicate, tr.object)) {
                found[operation](tr);
            }
            else {
                not_found[operation](tr);
