...
function PollingAgent(options,applyCsetCallback,regenerateCallback) {
  options = options || {};
  ...
  this._regenerateDb = regenerateCallback;
...
PollingAgent.prototype.pollCounting = function() {
  ...
  if(this._regenCounter >= this._regeneratingInterval) {
    var _this = this,
        afterRegenerate = function(){ console.log("Resuming polling after regen."); _this.startPolling(); };
    this.stopPolling(); // We stop polling until regenerating the file
    // Regenerate file
    this._regenerateDb(afterRegenerate);
    this._regenCounter = 0;
    return ;
  }
