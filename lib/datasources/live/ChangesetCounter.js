...
function ChangesetCounter(input) {
    ...
    date.setUTCFullYear(parseInt(input.year || input[0]) || 0);
    date.setUTCMonth((parseInt(input.month || input[1]) -1) || 0);
    date.setUTCDate(parseInt(input.day || input[2]) || 0);
    date.setUTCHours(parseInt(input.hour || input[3]) || 0);
...
ChangesetCounter.prototype.getHourPath = function() {
    return this._zeroPad(this._date.getUTCFullYear(),4) + "/" +
        this._zeroPad(this._date.getUTCMonth()+1,2) + "/" +
        this._zeroPad(this._date.getUTCDate(),2) + "/" +
        this._zeroPad(this._date.getUTCHours(),2) +"/";
};
