...
function ChangesetManager(options) {
    ...
    this._startPoint = new ChangesetCounter(options.lastCset);
    this.status = CSM_READY;
    this._HOUR_STEP = options.hour_step || 5; // Maximum number of hours to check per cycle
    ...
    this._afterListDownload = undefined;
    this._afterChangesetDownload = undefined;
    this._afterComputeOpList = undefined;
    this._afterApply = undefined;
    this._afterCleanup = undefined;
}
...
ChangesetManager.prototype.checkForChangesets = function() {
     ...
     */
    if(!this._afterListDownload) this._afterListDownload = this.downloadChangesets;
     ...
     */
    if(!this._afterChangesetDownload) this._afterChangesetDownload = this.computeOperationList;
    /* After having calculated the whole operation list, we apply them into the datasource
     */
    if(!this._afterComputeOpList) this._afterComputeOpList = this.applyChangesets;
    /* After applying a set of changes, we cleanup after ourselves.
     */
    if(!this._afterApply) this._afterApply = this.postApplyCleanup;
    /* TODO implement afterCleanup function */
    if(!this._afterCleanup) this._afterCleanup = this._finalizeOrStart;
        ...
        for(var j=0; j< files.length; j++) {
            ...
            cs.downloadAndParse(function(){
                ...
                if(_this._received == _this._awaiting) {
                    ...
                    _this._received = 0;
                    _this._awaiting = 0;
                    if(_this._afterChangesetDownload) {
                        setImmediate(function(){_this._afterChangesetDownload();});
                    }
                }
            });
        }
...
ChangesetManager.prototype.computeOperationList = function() {
    var cLists = this._changesetLists,
        _this = this;
    this._opList = new OperationList({callback:function(){_this._afterComputeOpList();}});
    this._opList.computeOperationList(cLists);
};

ChangesetManager.prototype._applyOperationList = function() {
    /* TODO - Implement */
    console.log("TODO: Implement applyOperationList!");
};
...
ChangesetManager.prototype._finalizeOrStart = function() {
    if(this._changesetLists && this._changesetLists.length > 0) {
        ...
        var _this = this;
        setImmediate(function() {_this._afterListDownload();});
    }
};
