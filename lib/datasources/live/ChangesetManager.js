...
function ChangesetManager(options) {
    ...
    this._HOUR_STEP = options.hour_step || 30; // Maximum number of hours to check per cycle
...
ChangesetManager.prototype.checkForChangesets = function() {
    ...
    if(!this._afterCleanup) this._afterCleanup = function(){};
...
ChangesetManager.prototype.computeOperationList = function() {
    var cLists = this._changesetLists,
        _this = this,
        opList = new OperationList({callback:function(ol){_this._afterComputeOpList(ol);}});
    setImmediate(opList.computeOperationList(cLists,0));
};
...
ChangesetManager.prototype.applyChangesets = function(oplist) {
    this._opList = oplist;
    if(this.status != CSM_NEXT_STEP) {
        console.log("Not ready to retrieve Changesets");
    }
