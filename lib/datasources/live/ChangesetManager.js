...
var startPointFile = 'latestChangeset.txt';

function ChangesetManager(options,applyCsetCallback) {
  options = options || {};
  ...
  this._HOUR_STEP = options.hour_step || 25; // Maximum number of hours to check per cycle
  this._CSET_THRESHOLD = options.changesetThreshold || 100; // Maximum number of changesets to apply per cycle
  this._applyCallback = applyCsetCallback;
  this._setLatestChangeset(options);
}

ChangesetManager.prototype._setLatestChangeset = function(options) {
  if(options.workspace && fs.existsSync(path.join(options.workspace,startPointFile))) {
    var fileName = path.join(options.workspace,startPointFile),
        fileCont = fs.readFileSync(fileName);
    console.log("Reading "+fileName+" | "+fileCont);
    this._startPoint = new ChangesetCounter(fileCont.toString());
  } else {
...
*/
ChangesetManager.prototype.checkForChangesets = function(callback) {
    this._finalizingCallback = callback;
    this.retrieveChangesetList(this._startPoint);
...
ChangesetManager.prototype._getChangesetListAsync = function(url,callback) {
    ...
    request(url,function(error,response,body) {
        console.log("REQUEST result came. status: "+(response ? response.statusCode : null)+" URL: "+url);
        _this._received++;
...
ChangesetManager.prototype._recordLastChangeset = function() {
  ...
  fs.writeFile(path.join(this._workspace,startPointFile),this._startPoint.getPath(),
...
ChangesetManager.prototype.downloadChangesets = function() {
    ...
    console.log("Downloading a bunch of changesets...");
    this.status = CSM_WAITING_CSETS;
    var csets = [],
        count = 0,
        _this = this,
        cd = new ChangesetDownloader();
    for(var i = 0; i < this._changesetLists.length; i++) {
        ...
        for(var j=0; j< files.length; j++) {
            ...
            var operation = this._getCsetOperation(files[j]),
                ...
                cs = {url: url, operation: operation};
                                ...
                                function(){
                                    ...
                                    if(_this._received == _this._awaiting) {
                                        ...
                                        if(_this.computeOperationList) {
                                            console.log("Computing operation list...");
...
ChangesetManager.prototype.computeOperationList = function() {
    var cLists = this._changesetLists,
        _this = this;
    this._opList = new OperationList({callback:function(){_this.applyChangesets();}});
    this._opList.computeOperationList(cLists);
};

ChangesetManager.prototype._applyOperationList = function() {
    /* TODO - Implement */
    var _this = this,
        afterApply = function(){_this.postApplyCleanup();};
    this._applyCallback(this._opList,afterApply);
};
...
ChangesetManager.prototype.finalizeOrStart = function() {
  ...
  console.log("Done. Polling again!");
  if(this._finalizingCallback) this._finalizingCallback();
};

ChangesetManager.prototype.postApplyCleanup = function() {
    console.log("Doing some cleanup after Applying... ");
    if(this.status != CSM_NEXT_STEP) {
        console.log("Not ready to retrieve Changesets");
    }
    if(this._opList.csListsIdx >= this._changesetLists.length) {
        ...
        this._changesetLists = this._changesetLists.slice(this._opList.csListsIdx);
        if(this._changesetLists[0].csets && this._changesetLists[0].csets.length) {
            this._changesetLists[0].csets = this._changesetLists[0].csets.slice(this._opList.cSetPos);
        }
        if(this._changesetLists[0].files && this._changesetLists[0].files.length) {
            this._changesetLists[0].files = this._changesetLists[0].files.slice(this._opList.cSetPos);
        }
    }
    delete this._opList;
