...
ChangesetManager.prototype._setLatestChangeset = function(options) {
  if(!fs.existsSync(path.join(this._workspace,startPointFile))) {
    this._startPoint = new ChangesetCounter(options.latestChangeset);
    return ;
  }
  this._startPoint = new ChangesetCounter(fs.readFileSync(path.join(this._workspace,startPointFile)));
};
