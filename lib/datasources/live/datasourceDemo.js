...
var options = {file:'test.hdt',
               workspace:'./workspace/'}; 

var lhds = LiveHdtDatasource(options),
    printer = function(src) { if(lhds._auxiliary[src]) lhds._auxiliary[src].get({},function(err,list){console.log(list);});};
 
// Wait a few seconds before applying
lhds.applyOperationList(opList,function(){printer('added');});

var res = [];
lhds._executeQuery({},res,undefined);

// Wait a few seconds before applying
opList = {added:opList.removed.slice(), removed:opList.added.slice()};
lhds.applyOperationList(opList,function(){printer('added');});

opList = {added:removeList.slice(), removed:[]};
lhds.applyOperationList(opList,function(){printer('added');});
