...
function LiveHdtDatasource(options) {
  ...
  this._initializeAuxiliaryStores(options,hdtFile);
...
LiveHdtDatasource.prototype.rebuildHdtDatabase = function(callback) {
  ...
  console.log("Regenerating database with args:"+[this._auxiliaryPaths.added, this._auxiliaryPaths.removed, 
                                                        this._auxiliaryPaths.hdt, newHdt]);
  rebuilder.on('exit',function(code) {
    if(code !== 0) return;
    _this._auxiliaryPaths.hdt = newHdt;
    _this._auxiliary.removedBloom = new Bloem(1674918,2); // With this, we can clean up the auxiliaries
    _this.loadHdtDocument(newHdt);
    var dbCleaner = ChildProcess.spawn("rm",["-r",_this._auxiliaryPaths.added,_this._auxiliaryPaths.removed]);
    dbCleaner.on('exit',function(code) {
      _this.initializeAuxiliaryStores({added:_this._auxiliaryPaths.added,removed:_this._auxiliaryPaths.removed},newHdt);
...
LiveHdtDatasource.prototype._initializeAuxiliaryStores = function(options,hdtFile) {
  ...
  this._auxiliaryPaths = {added: add_path, removed: remove_path,hdt:hdtPath};
  this._auxiliary = {added: levelgraph(levelup(add_path)),
                     removed: levelgraph(levelup(remove_path)),
                     removedBloom: new Bloem(1674918,2)}; // Size matching a SafeBloem(500 000, 0.2)
  ...
  removedStream.on('end',function() {
    ...
    _this._pollingAgent.startPolling();
  });
};
...
LiveHdtDatasource.prototype.applyOperationList = function(opList,callback) {
  ...
  var _this = this,
      ...
      callbackCounter = function() {
        callbackCount += 1;
          console.log("Added "+callbackCount+" to the auxiliaries...");
  ...
  countRemove = function() {
    removeCount += 1;
    if(removeCount == 2) {
      console.log("Removed intersections, now adding to stores...");
      _this._addToBloom(_this._auxiliary.removedBloom, opList.removed);
      console.log("Added to the bloom filter...");
      _this._offsetCache.flushCache(); // We need to flush our cache, because we have updates
      console.log("Flushed the offset cache!");
      _this._addAll(_this._auxiliary.added, opList.added,function() {
        callbackCounter();
