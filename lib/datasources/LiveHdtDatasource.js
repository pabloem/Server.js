...
LiveHdtDatasource.prototype._afterHdtTriples = function(statusDic,query,tripleStream,metadataCallback,
                                                        limit, tripleCount) {
  /*Case 0.1: We have not served any triples, nor have we reached the end of HDT file // TODO
         Conditions - tripleCount == limit and servedTriples == 0
    Case 0.2: We have served some triples, and have not reached the end of HDT file   // TODO
         Conditions - tripleCount == limit and servedTriples > 0
   */
  if(limit && tripleCount == limit && statusDic.servedTriples < query.limit) { // Case 0
    ...
    tripleStream.push(null); // For now, we just finish
    return ;
  }
      ...
      for(var i = 0; i < tripleCount; i++) {
        _this._computeHdtRemoved(countDic,triples,remStore,i,function(){
          ...
          for(var i = 0; i < tripleCount; i++) {
            ...
            if((!offset || counter > offset) && (!query.limit || statusDic.servedTriples < query.limit)) {
          ...
          //console.log("Counter: "+counter+" | served: "+statusDic.servedTriples+" | i: "+i+
          //" | OffDic: "+JSON.stringify(offDic));
