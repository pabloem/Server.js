...
var hdtFileLocation = 'hdtFileLocation.txt',
    ...
    doneMessage = 'done',
    startPollingMessage = 'startPolling',
...
function LiveHdtDatasource(options) {
  ...
  if(cluster.isMaster || cluster.worker.id == 1) {
    ...
    /* NOTE: We do this on startup, to select a default polling worker, but 
       this is not the general way in which we choose polling workers */
...
LiveHdtDatasource.prototype.setupParallelConfig = function(options) {
  ...
  process.on('message',function(msg) {
    ...
    if(msg.type == doneMessage) {
      _this.receivedDoneMessage(msg);
    }
    ...
    if(msg.type == startPollingMessage) {
      if(_this._pollingAgent) {
        log4js.error("A process with PollingAgent has been instructed to create one - this is not supposed to happen.");
        return ;
      }
      _this._pollingAgent = new PollingAgent(options,
                                             this.applyOperationList.bind(this),
                                             this.rebuildHdtDatabase.bind(this));
    }
  });
...
LiveHdtDatasource.prototype.getNewAuxiliaries = function(msg) {
  // This message is supposed to come to the non-polling workers. If a polling worker receives it, it's an error.
  if(this._pollingAgent) {
...
LiveHdtDatasource.prototype._initializeAuxiliaryStores = function(addedTriplesDb,removedTriplesDb,callback) {
  ...
  // The polling process will keep track of the auxiliary databases
  if(this._pollingAgent) {
