...
LiveHdtDatasource.prototype._computeHdtRemoved = function(countDic,triples,removedStore,idx,callback) {
  ...
  this._auxiliary.removed.get(triples[idx],function(e,l) {
    countDic.got += 1;
    if(l.length > 0) removedStore.addTriple(triples[i].subject,triples[i].predicate,triples[i].object);
...
LiveHdtDatasource.prototype._returnEstTotalCount = function(query,hdtCount,servedTriples,metadataCallback) {
...
LiveHdtDatasource.prototype._getHdtTriples = function(query,tripleStream,metadataCallback) {
  var _this = this,
      offDic = this._offsetCache.getClosestLowerOffset(query,query.offset),
      limit = this._probabilisticGetLimit(query.limit,query.offset - offDic.label),
      realOffset = offDic.offset,
      offsetLabel = offDic.label;
      ...
      for(i = 0; i < tripleCount; i++) {
        _this._computeHdtRemoved(countDic,triples,remStore,i,function(){
          ...
          for(var i = 0; i < tripleCount; i++) {
            ...
            if(limit && counter % limit == 0) this._offsetCache.addToCache(query,counter,realOffset+i);
            ...
            if((!offset || counter >= query.offset) && (!query.limit || servedTriples < query.limit)) {
              servedTriples += 1;
              tripleStream.push(triples[i]);
            }
            /* If we have a limit, and we reached it, then we break out */
            if(query.limit && servedTriples == query.limit) break;
          }

          if(servedTriples == query.limit) { // We are done serving triples.
            tripleStream.push(null);
            return ;
          }
          if(!limit && tripleCount < limit) { // We are at the limit of the hdt file.
            tripleStream.push(null);
            // Add added triples (from the auxiliary store)
            return ;
          }
          // OTHERWISE, WE NEED TO QUERY AGAIN! - We do nothing - for now.
          _this._returnEstTotalCount(query,estimatedTotalCount,servedTriples,metadataCallback);
          tripleStream.push(null);
          return ;
        });
      }
...
LiveHdtDatasource.prototype._executeQuery = function (query, tripleStream, metadataCallback) {
  ...
  this._getHdtTriples(query,tripleStream,metadataCallback);
};
